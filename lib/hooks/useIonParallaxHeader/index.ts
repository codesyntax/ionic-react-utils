...
export function useIonParallaxHeader({
  ...
  const initElements = () => {
    ...
    if (ref && ref.current) {
      ...
      if (!toolbar)
        throw new Error(
          'Parallax requires an <ion-toolbar> element on the page to work',
        );
      ...
      // .scroll-content
      if (scrollContent) {
        scrollContent.style.paddingTop = `${
          imageOverlay.clientHeight - toolbar.clientHeight
        }px`;
            ...
            window.requestAnimationFrame(() => {
              ...
              if (barButtons && showBarButtons) {
                if (targetHeight > headerMinHeight) {
                  ...
                  endBarButtons && imageOverlay.append(endBarButtons);
                  ...
                  endBarButtons && toolbar.append(endBarButtons);
